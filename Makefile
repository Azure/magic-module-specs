...
build-terraform:
	...
	@if [ -z "$(RESOURCE)" ]; then \
		...
		cd $(MMROOT) && \
		@bash $(ROOT)/list_resources.sh | xargs -I '{}' bundle exec compiler -d -p $(SPECROOT)/'{}' -e terraform -o $(TFROOT)/$(TFREPO)/; \
	else \
...
build-ansible:
	...
	@if [ -z "$(RESOURCE)" ]; then \
		...
		cd $(MMROOT) && \
		@bash $(ROOT)/list_resources.sh | xargs -I '{}' bundle exec compiler -d -p $(SPECROOT)/'{}' -e ansible -o $(GENROOT)/$(ASREPO)/; \
	else \
